import { rapportsRenseignement } from '../../data/mockData'
  ...
  return (
    <section className="grid min-h-0 grid-rows-[auto_1fr] gap-3.5">
      ...
      <div className="grid grid-cols-3 gap-3.5 overflow-auto">
        {rapportsRenseignement.map((rapport) => (
          <article key={rapport.id} className="grid gap-3 rounded-lg border border-[#d8ded9] bg-white p-3.5 shadow-sm">
            <span className={`inline-flex w-fit min-h-[26px] items-center rounded-full px-2.5 text-xs font-bold ${statutStyle[rapport.statut].badge}`}>
              {statutStyle[rapport.statut].label}
            </span>
            ...
            <span className="text-xs text-[#65706a]">Classification : {classificationLabel[rapport.classification]}</span>
          </article>
        ))}
      </div>
    </section>
